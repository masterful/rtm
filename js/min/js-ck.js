//specify the order of imports (that's all this file's for, really)

//@codekit-append "assumptions.js"
//@codekit-append "jquery.js"
//@codekit-append "common.js"

//@codekit-append "md5.js"
//@codekit-append "db.js"
//@codekit-append "rtm.js"

//@codekit-append "on-load.js"

/*********************************************** 
     Begin assumptions.js 
***********************************************/ 
...
if ( ! alert ) {		var alert		= function() {}; }

//and things I shouldn't be redefining, but will really hurt:
if ( ! openDatabase ) { alert('No Database available'); }
...
(function($) {
	$.showError	= function( name, error ) {
		var c = $('<div>', { 'class': 'error' }).appendTo($('#errors')).fadeIn(400).delay(6000).fadeOut(400);
...
var db	= {
	...
	, _tables	: {
		setting		: {
			title		: 'setting'
			, desc		: 'The app settings'
			, schema	: '( s_id INTEGER PRIMARY KEY ASC, name TEXT, value TEXT )'
			, key		: 's_id'
		}, frob		: {
			title		: 'rtm_frob'
			, desc		: 'The frob(s) used to get tokens'
			, schema	: '( f_id INTEGER PRIMARY KEY ASC, value TEXT )'
			, key		: 'f_id'
		}, token	: {
			title		: 'rtm_token'
			, desc		: 'The token(s) used'
			, schema	: '( tk_id INTEGER PRIMARY KEY ASC, f_id INTEGER, value TEXT, valid BOOLEAN )'
			, key		: 'tk_id'
		}, list		: {
			title		: 'rtm_list'
			, desc		: 'The various lists'
			, schema	: '( l_id INTEGER PRIMARY KEY ASC, name TEXT )'
			, key		: 'l_id'
		}, task		: {
			title		: 'rtm_task'
			, desc		: 'The tasks'
			, schema	: '( t_id INTEGER PRIMARY KEY ASC, name TEXT )'
			, key		: 't_id'
		}
	}
	...
	, connect	: function() {
		...
		$.each(this._tables, function( table, info ) {
			self._conn.transaction( function( tx ) {
				tx.executeSql('CREATE TABLE IF NOT EXISTS ' + info.title + info.schema, []);
			});
		});
	}
	//transaction wrapper
	...
	, update	: function( table, item, values ) {
		var	self	= this
			, query	= ''
		...
		query	= 'UPDATE ' + info.title + ' SET ' + query.substr(1) + ' WHERE ' + info.key + '=?';
		a.push(item[info.key]);
		self.trans(function(tx) {
			tx.executeSql( query, a, self.success, self.error );
		});
	}
	...
	, insert	: function( table, values, callback ) {
		...
		query	= 'INSERT INTO ' + info.title + ' (' + query.substr(1) + ') VALUES (' + qs.substr(1) + ')';
		self.trans(function(tx) {
			tx.executeSql( query, a, (callback) ? callback : self.success, self.error );
		});
	}
...
var RTM	= {
	...
	//we'll need this array for when we do callbacks:
	, callCount	: 0
	
	//authenticate
	, authenticate	: function( stage ) {
		var self	= this;
		switch( stage ) {
		case 'use_frob':
			$.debug('using frob');
		...
		case 'new_token':
			...
			self['callback'+(self.callCount)]	= function( data ) {
				if ("ok" !== data.rsp.stat) {
					$.showError("Couldn't authenticate with RTM");
					return;
				}
				//insert frob into database, and display link
				db.insert('frob', { value: data.rsp.frob }, function(tx, rs) {
					//make 2 links on page:
					$('#modals').append(
						$('<div>',{'class':'modal'}).append(
							$('<a>', { 'class': 'button', href: self._auth_url + self.serialize({
									perms	: 'delete'
									, frob	: data.rsp.frob
								}), target: '_blank'
							}).html('Click here')
						).append(
							' to authenticate, and then come back here and '
						).append(
							$('<a>', { 'class': 'button', href: '#' }).on('click', function(e) {
								e.preventDefault();
								self.authenticate('use_frob');
							}).html('click this')
						).append('.')
					).fadeIn(300);
				});
			...
			self.call({
				...
				, callback		: 'RTM.callback' + self.callCount++
			});
			break;
		default:
			...
			db.select(
					...
					, function(tx, rs) {
				var token	= ( rs && rs.rows && 0 < rs.rows.length ) ? rs.rows.item(0) : null;
				if ( token ) {
					//we'll need a callback:
					self['callback'+(self.callCount)]	= function( data ) {
...
$(function() {
	$('a.authenticate').on('click', function(e) {
		e.preventDefault();
		RTM.authenticate();
	});
});